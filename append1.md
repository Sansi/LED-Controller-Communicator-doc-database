...
|58|终端自动调光|控制器地址,终端地址,自动表ID`\n`||
|59|终端时控调光|控制器地址,终端地址,时控表ID`\n`||
|60|设置紧急调光模式|控制器地址,模式|0：终端故障亮度，1：最近亮度，2：预存时控计划。通信时复用 `0x000D` 指令，用 `0xA0`, `0xA1` 和 `0xA2` 表示上述模式。 注意全局模式应统一，这里参数加控制器地址仅为了方便客户端识别操作失败的控制器。|
|61|传感器状态查询|传感器ID`\n`| |
